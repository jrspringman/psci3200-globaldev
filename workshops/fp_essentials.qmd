...
ggplot(prediction_df %>% filter(!is.na(year)), 
       aes(x = year, y = prediction, color = factor(moved), shape = factor(year))) +
  geom_point(size = 4) + 
  geom_text(aes(label = round(prediction, 2)), vjust = -1.2, size = 4) +
  scale_x_continuous(breaks = c(0, 1), labels=c("Y1", "Y3")) +
  expand_limits(y = c(.2, .2)) +
  labs(x = "Year", y = "Predicted z_participation_base", color = "Moved", shape = "Year") +
